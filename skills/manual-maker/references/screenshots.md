...
- **Derive the circles from the _final_ step list — never draw before the steps are frozen.** This is
  ...
  earlier draft, then the steps rewritten and the circles never re-derived. The delivered manual had
  ② on the control for step 3, ④ on an *alternative* path that was not a numbered step at all, and
  ⑤ drawn on two different figures. **If the step list changes, re-derive every circle in that
  section** — do not patch one number and assume the rest still line up.
...
Annotation emits a manifest **beside the images**, so the 1:1 claim can be *proved* rather than
eyeballed. `scripts/verify-annotations.py` reads it at Step 8 and compares it with the actual pixels.

```json
{"sections": {"2.1": {"steps": 7, "circles": [
   {"n": 3, "file": "02-1-02.png", "x": 549, "y": 1180,
    "label": "ปุ่มเข้าห้องเรียน",
    "step_text": "คลิกปุ่ม “เข้าห้องเรียน”"}
]}}}
```

| คีย์ | ความหมาย |
|---|---|
| `<section>` | เลขหัวข้อย่อย เช่น `"2.1"` — ตรงกับเลขที่ main thread แจกมา |
| `steps` | **จำนวนขั้นตอนที่มีเลขของหัวข้อนั้น** ตามดราฟต์สุดท้าย |
| `circles[].n` | เลขในวง = เลขขั้นตอน ต้องครบ **1..steps** ไม่ขาด ไม่ซ้ำ ไม่เกิน |
| `circles[].file` | ชื่อไฟล์ใน `manual-assets/<slug>/` (ชื่อล้วน ไม่ใช่ path) |
| `circles[].x`,`y` | **จุดกึ่งกลางวง ในพิกเซลของ PNG ไฟล์สุดท้าย** (หลังใส่วงแล้ว ไม่ใช่ภาพก่อนแต่ง) |
| `circles[].label` | เป้าที่วงทับอยู่ เขียนด้วยคำที่ระบบใช้จริง เช่น `"ปุ่มบันทึก"` |
| `circles[].step_text` | **ข้อความของขั้นตอนที่ `n` นี้ คัดลอกมาตรง ๆ จากดราฟต์** |

**`step_text` บังคับทุกวง** — ถ้าไม่มี ชั้นที่ 3 พิสูจน์ไม่ได้ และ **ตรวจไม่ได้ = ไม่ผ่าน**

`label` กับ `step_text` คือคู่ที่ทำให้ตรวจการจับคู่เชิงความหมายได้: คู่มือมีธรรมเนียมของตัวเองอยู่แล้ว
(บังคับใน `template.md`) ว่า **ขั้นตอนต้องอ้างชื่อคอนโทรลจริงในเครื่องหมายคำพูด** เช่น
`คลิกปุ่ม “เข้าห้องเรียน”` สคริปต์จึงดึงชื่อคอนโทรลในเครื่องหมายคำพูดออกมา แล้วบังคับว่า `label`
ต้องพูดถึงคอนโทรลตัวเดียวกัน (เทียบหลังตัดคำนำหน้าอย่าง ปุ่ม/เมนู/แท็บ/ช่อง/ไอคอน ออก
`"ปุ่มเข้าห้องเรียน"` จึงแมตช์กับ `“เข้าห้องเรียน”` ได้)

**ขั้นตอนที่ไม่ได้อ้างคอนโทรลในเครื่องหมายคำพูดเลย** (เช่น `เปิดเบราว์เซอร์ แล้วเข้าที่ https://…`
หรือ `คลิกคาบเรียนที่ต้องการในตาราง`) **ข้ามไป ไม่ใช่ตก** — บังคับไม่ได้ในเมื่อไม่มีอะไรให้เทียบ
**ยกเว้น**กรณีเดียว: `label` ดันไปตรงกับคอนโทรลที่ *ขั้นตอนอื่นในหัวข้อเดียวกัน* อ้างไว้ → **ตก**
เพราะนั่นคือลายเซ็นของบั๊กตัวจริง (วงถูกวางตามดราฟต์เก่า เลยไปนั่งบนคอนโทรลของขั้นตอนข้างเคียง)

**งานขนาน (Step 4–6):** ผู้เขียนแต่ละคนเขียน **ไฟล์ของตัวเอง**
`manual-assets/<slug>/<section>.annotations.json` (เลขหัวข้อ = เลนความเป็นเจ้าของเดิม ตาม `parallel.md`)
ห้ามเขียนไฟล์รวมพร้อมกัน — สคริปต์รับ **โฟลเดอร์** แล้วรวมให้เอง และฟ้องถ้าสองคนอ้างหัวข้อเดียวกัน

**ถ้าโหมดภาพเป็น `none`** (ผู้ใช้สั่งไม่ให้มีวง) ก็ไม่มีแมนิเฟสต์และไม่ต้องรันสคริปต์นี้ —
สิ่งที่ต้องพิสูจน์แทนคือ *ไม่มีวงเลย* ซึ่งอยู่ในชั้นที่ 1 ของ `review.md`
