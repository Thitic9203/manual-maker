...
**สถานะ:** สร้างครบ 21 หน้าหลัก + 28 หน้าต่อเนื่องแล้ว (2026-07-21) · **ยังไม่ผ่านรีวิวชั้น 5 (render)** — ดู §0 · ✅ **ปลดล็อกหยุดงานแล้ว 2026-07-22 รอบบ่าย — MICA2-629 เนื้อหากลับมาปกติ ดู §0.4**
**วันที่:** 2026-07-20 (เริ่มแผน) · สำรวจสถานะล่าสุด 2026-07-22 (รอบบ่าย — สำรวจสดซ้ำผ่าน `/confluence-docs`)
...
| EvMS | MICA2-635 | Create Exam (AI) | [3712090246](https://skilllane.atlassian.net/wiki/spaces/PLUT/pages/3712090246) | 0 | ยังไม่มี Story ที่ผ่านเกณฑ์ | **เช็คซ้ำวันนี้ — ยังไม่มี Story เข้าเกณฑ์จริง** · ✅ ชื่อหน้าเป็น `(AI)` ตาม Jira แล้ว (ตรวจสด รอบบ่าย) — ปิดประเด็น |
...
| ELMS | MICA2-125 | AI Integration | [3711795429](https://skilllane.atlassian.net/wiki/spaces/PLUT/pages/3711795429) | 3 | ตรวจรายใบขณะจัดทำ | ✅ **เติม 3 UC แล้ว** (UC-1 MICA2-694 · UC-2 MICA2-585 · UC-3 MICA2-584) — ตรวจสดรอบบ่าย · เหลือรีวิวชั้น 5 |
...
| ELMS | MICA2-629 | Google Connect | [3712024773](https://skilllane.atlassian.net/wiki/spaces/PLUT/pages/3712024773) | 3 | ตรวจรายใบขณะจัดทำ | ✅ **เนื้อหากลับมาปกติแล้ว** (UC-1 MICA2-37 · UC-2 MICA2-112 · UC-3 MICA2-885) — ตรวจสดรอบบ่าย ดู §0.4 · เหลือรีวิวชั้น 5 |

**รวม 18 Epic · 295 UC** (292 เดิม + 3 UC ใหม่ของ MICA2-125) · CBMS 38 · EvMS 155 · ELMS 102 · ไม่มี Epic ใดขึ้นสถานะ "ตรวจแล้ว ไม่พบจุดแก้" (ผ่านครบ) แม้แต่หน้าเดียว

### 0.2 งานที่ต้องทำต่อ ภาพรวม (ข้ามทุกระบบ)

1. **รีวิวชั้น 5 (render บนหน้าเผยแพร่จริง)** — ยังไม่ทำเลยทั้ง 21 หน้าหลัก (ตาม panel-warning บนทั้ง 3 หน้าดัชนี) ← **งานหลักที่เหลือ**
2. ~~MICA2-125 เติม 3 UC~~ — ✅ **เสร็จแล้ว** (ตรวจสด 2026-07-22 รอบบ่าย)
3. **แปลง `รอข้อมูล` → `TBC` แดง ทั้งชุด 49 หน้า** — ผู้ใช้สั่งรอบบ่าย 2026-07-22 (ดู §0.5) · **ยังไม่เขียน**
4. **หน้าต่อเนื่อง 28 หน้า** (EvMS: MICA2-630/632/633 · ELMS: MICA2-626/627/628) — ยุบรวมเป็นหน้าเดียวได้ด้วยวิธี create+update ทีละก้อน (พิสูจน์แล้วกับ MICA2-631) แต่ต้องลบหน้าต่อเนื่องทิ้งซึ่งย้อนกลับไม่ได้ → **ไม่ทำ** (ดู §0.5) คงหน้าต่อเนื่องไว้ตามเดิม
5. ~~ชื่อหน้า MICA2-635~~ — ✅ ปิดแล้ว หน้าจริงเป็น `(AI)` ตาม Jira
6. **Theme MICA2-652**: Jira "Credit Transfer Review" / บอร์ด "Credit Transfer (Staff)" — คงชื่อบอร์ด ช่องที่ขัดกัน = TBC รอ BA
7. **ประเด็นที่ต้องให้ BA ชี้ขาด (จาก §7.3 เดิม ยังไม่มีคำตอบ)**: MICA2-738/739, MICA2-418/433 vs 462, MICA2-681 vs 809, MICA2-119/120, MICA2-9, MICA2-33/34, MICA2-92, MICA2-69/70/173, Story ว่าง 20 ใบรวมทุกระบบ → ระหว่างรอ ช่องที่ผูกอยู่ = TBC

### 0.5 ข้อตัดสินใจของผู้ใช้ 2026-07-22 (รอบบ่าย)

> "จัดมา ส่วนอะไรที่ตัดสินใจเองไม่ได้ เช่นรอ PO ก็ใส่ TBC ไว้"

| ประเด็น | สรุป |
|---|---|
| ค่าที่ AI ตัดสินเองไม่ได้ (รอ PO / รอ BA / Jira ไม่มีข้อมูล) | ใส่ **`TBC` status สีแดง** — นโยบายเดียวกับ OLS รอบ 3 |
| ขอบเขตการแปลง | ทุก `<span data-type="status">รอข้อมูล</span>` ใน **49 หน้าของงานนี้** (21 หลัก + 28 ต่อเนื่อง) · ข้อความอธิบายท้าย span (เช่น "— Story ไม่ได้ระบุ") **คงไว้** |
| หน้า OLS 17 หน้า | **นอกขอบเขตรอบนี้** — เป็น action 12 ของ [ols-prd-generation-progress.md](ols-prd-generation-progress.md) |
| ยุบ 28 หน้าต่อเนื่อง | **ไม่ทำ** — ต้องลบหน้าถาวร ย้อนกลับไม่ได้ AI ไม่ตัดสินใจแทน (กติกา §4 + rule ห้ามทำลายของเดิม) รอผู้ใช้สั่งตรงถ้าต้องการ |
| MICA2-652 theme | คง `Credit Transfer (Staff)` ตามบอร์ด · ช่องที่ขัดแย้ง = TBC รอ BA |

**Transform contract (ตรึง — verifier บังคับ):**

```
<span data-local-id="…" data-type="status" data-color="neutral">รอข้อมูล</span>
  →  <span data-local-id="…" data-type="status" data-color="red">TBC</span>
```

- `data-local-id` คงเดิมทุกตัว · byte นอก status span ต้องเหมือนเดิมทั้งหมด
- สคริปต์ `tbc.py --verify ORIG NEW` exit 1 = **ห้ามเขียน**
- เขียน **ทีละหน้า sequential** · เช็ค `id` ใน response · **อ่านกลับทุกหน้าหลังเขียน** (ไม่เชื่อ response — บทเรียน §7.1)

### 0.4 ✅ ข้อมูลเสียหายที่พบ 2026-07-22 (เช้า) — คลี่คลายแล้ว (บ่าย)
...
**สถานะ: ✅ คลี่คลายแล้ว — ตรวจสด 2026-07-22 รอบบ่าย** หน้า `3712024773` มีเนื้อหา Google Connect ของตัวเองครบ (UC-1 MICA2-37 · UC-2 MICA2-112 · UC-3 MICA2-885) ไม่ใช่สำเนาหน้าดัชนี OLS แล้ว · `lastModified` 2026-07-22T08:16Z

**สุ่มตรวจหน้าอื่น (ข้อ 3 เดิม) — ผ่าน:** ไล่ `getConfluencePageDescendants` ครบทั้ง 3 ระบบ → 49 หน้า ชื่อหน้าถูกต้องทุกหน้า ตรงกับ §7 ไม่พบหน้าอื่นที่เนื้อหาสลับ

**บทเรียนที่ต้องคงไว้:** หน้า 3 ใบ (`3711795429`, `3712024773`, `3712090246`) ถูกแก้ภายใน 4 นาทีจาก session อื่นในวันเดียวกัน — **การเขียนพร้อมกันหลาย session/agent คือเงื่อนไขที่ทำให้เกิด crosstalk** ตั้งแต่รอบนี้ไป: เขียน **sequential ในเธรดหลักเท่านั้น ห้าม fan-out agent เขียนพร้อมกัน** และอ่านกลับยืนยันทุกหน้า
...
> อัปเดต 2026-07-22 (บ่าย): ข้อ 2 ปิดแล้ว · ข้อ 1 และ 3 ตัดสินแล้วใน §0.5 · เหลือข้อ 4 เป็นงานจริง

1. ~~หน้าต่อเนื่อง 28 หน้า~~ — **ตัดสินแล้ว §0.5: ไม่ยุบ ไม่ลบ** (ลบถาวร ย้อนกลับไม่ได้)
2. ~~ชื่อหน้า MICA2-635~~ — ✅ ปิด หน้าจริงเป็น `Create Exam (AI)` ตาม Jira
3. **Theme vs title คนละแหล่ง** — MICA2-652 Jira ว่า `Credit Transfer Review` บอร์ดว่า `Credit Transfer (Staff)` → **คงชื่อบอร์ด ช่องที่ขัดกัน = TBC รอ BA**
