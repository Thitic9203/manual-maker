...
**Do not oversell that script, and do not let anyone "simplify" its detector.** It proves *numbering and placement consistency* only — numbers contiguous 1..N, no duplicates, none out of range, and every claimed coordinate landing on a circle that really exists. **Whether a circle sits on the _correct_ control is semantic and stays human judgement**; ② pointing at step 3's button with an honest coordinate passes the script, which is why `review.md` layer 3 still demands a human table keyed on `label` and why ตรวจไม่ได้ = ไม่ผ่าน still applies. The detector requires a **near-solid disc of the expected size** (four conditions: bbox ~2r square, aspect 0.80–1.25, fill 0.58–0.86, area within the disc band) rather than "connected red pixels", because the app's own UI is full of red — measured on the 19 real ELMS screenshots, callouts are bbox 37×37 / area 931–1001 / fill 0.68–0.73 / aspect 1.00, while red UI chrome came in at 59×20 (aspect 2.95), 38×20 (fill 0.726 — so **fill alone does not discriminate**), and a roundish 26×24 delete icon at area 325. Every decoy fails at least two conditions; a single-condition filter would not have separated them. Geometry was stable across colour tolerance 20→80 (bbox unchanged, centroid drift < 0.2 px), which is why the 25 px match radius is generous rather than tuned.
