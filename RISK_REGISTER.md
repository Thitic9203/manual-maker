...
## MM-005 — User-scope self-update hook survives the plugin being disabled (edits `~/.claude/settings.json`)

- **Date:** 2026-07-31
- **Decision by:** repo owner (explicit choice, this session: "ติดตั้งอัปเดตสกิลให้ผู้ใช้งานอัตโนมัติให้ได้ … โดยที่เขาไม่ต้องรันไรเลย")
- **Status:** Accepted, shipped in v0.24.0

**What changed.** While the plugin is enabled, `hooks/check-version.sh` now also registers a **user-scope** SessionStart hook in `~/.claude/settings.json` that runs `hooks/self-update.sh` (installed to `~/.manual-maker/self-update.sh`). A user-scope hook runs on every session regardless of any plugin's enabled state, so it keeps the install current **even after the plugin is disabled** — closing a silent freeze that a plugin-owned hook structurally cannot.

**Why.** Claude Code does not run a *disabled* plugin's hooks, so `check-version.sh` — the whole auto-update path (MM-001) — stops the moment the install is disabled, and the install stays pinned at whatever version it was disabled at, silently (seen in the wild: an install stuck at 0.22.0 with the skill missing from the session; it took a manual `enable` + `update` to recover). The only mechanism that survives a disable is a hook the plugin does **not** own — i.e. one registered at user scope in `settings.json`. It still has to be bootstrapped from the plugin's own hook while enabled at least once; there is no way to place code on a machine running none of your code, by Claude Code's security design.

**Behaviour of `self-update.sh` (deliberately narrow).** It only *acts* when the install is **disabled and behind** — then it enables + updates in the background and notifies. Enabled → silent (MM-001's hook already handles it; no double update, shared lock). Up to date → silent (a user who disables an up-to-date install is left alone; the trigger is a *new version* only, so there is no re-enable loop). Auto-enabling a disabled install is the owner's explicit choice for an internal team tool whose point is a zero-touch, always-current install; `MANUAL_MAKER_NO_AUTOUPDATE=1` stops the whole thing.

**Trade-offs accepted.**
1. **The plugin edits the user's `settings.json`** — the highest-blast-radius thing it does (a corrupted `settings.json` breaks the user's whole Claude Code). Mitigated: the JSON surgery is a **separate, unit-tested** script (`hooks/install-user-hook.py`, 7-case fixture test — missing / empty / foreign-hooks / idempotent / path-change / invalid-JSON / backup) that touches only its own marker-tagged entry, refuses to write if the file is not valid JSON or has a foreign hooks shape, writes atomically (temp + `os.replace`), and keeps a one-time `settings.json.mm-bak`.
2. **Runs on every session, globally (all projects), forever** — a user-scope hook is not scoped to manual-maker work. Mitigated by a 6-hour throttle (stamp in `~/.manual-maker/state/`), a 3-second network cap, fail-silent everywhere, and a detached background update so session start never blocks.
3. **A self-healing updater that outlives `disable`** can read as user-hostile. Mitigated by acting only on a *new version* (no loop), the `MANUAL_MAKER_NO_AUTOUPDATE=1` opt-out, and never force-enabling an up-to-date disabled install.
4. **Not removed by `/plugin uninstall`** (same as MM-003) — the plugin can't run once gone, so `self-update.sh` then finds no install and exits silently; the `settings.json` entry and `~/.manual-maker/self-update.sh` are left behind. Documented removal in the README.

**Safety rails (keep these).** `install-user-hook.py`: marker-scoped edits only, refuse-on-unparseable, atomic write, one-time backup. `self-update.sh` + Job 3 in `check-version.sh`: opt-out `MANUAL_MAKER_NO_AUTOUPDATE=1`, `/usr/bin/python3` required (skip if absent), fail-silent on every step, single-flight lock **shared** with `check-version.sh`, supported `claude plugin` CLI only, 6-hour throttle, refresh the installed copy only when it differs. All proven end-to-end in an isolated `HOME` (copy + `chmod +x`, settings entry created, idempotent on re-run, no real update fired at equal version).

**How to reverse.** Per user: set `MANUAL_MAKER_NO_AUTOUPDATE=1`, remove the `# manual-maker-plugin managed self-update` entry from `~/.claude/settings.json` (or restore `~/.claude/settings.json.mm-bak`), and `rm ~/.manual-maker/self-update.sh`. In the repo: delete `hooks/self-update.sh`, `hooks/install-user-hook.py`, and the `install_self_update_hook` block in `hooks/check-version.sh`.
