...
![version](https://img.shields.io/badge/version-0.17.0-blue) ![license](https://img.shields.io/badge/license-MIT-green) ![Claude Code](https://img.shields.io/badge/Claude%20Code-plugin-8A2BE2)
...
**Version 0.17.0 · MIT · Claude Code plugin**
...
| ความเร็วตอนทำ | **ขนานต่อหัวข้อย่อย (v0.17.0+)** — writer 2–3 ตัว + reviewer 1 ตัว | 1 หัวข้อย่อย = 1 คนทำครบ ถ่ายภาพ+วงแดง+เขียนสเตป (กฎเลขวง=เลขขั้นตอน 1:1 อยู่ในหัวข้อ แยกคนแล้วพัง) แต่ละคนเขียนได้เฉพาะไฟล์ของหัวข้อตัวเอง จึงไม่ชนกัน · reviewer รีวิวทันทีทีละหัวข้อย่อย **เจอเรื่องต้องใช้ดุลพินิจ = ถามผู้ใช้ก่อนเสมอ** |
| การตรวจก่อนส่ง | **รีวิว 5 ชั้น + gate (v0.16.0+)** — ไม่ผ่านครบ 5 ชั้น = ไม่ส่ง | ตรวจจากไฟล์ที่ export แล้ว: ตรงตามที่ยืนยัน · ทุกอย่างมีที่มา · ภาพ · เลข/คำ (รวม**คำพราก**) · รูปเล่ม. **ตรวจไม่ได้ = ไม่ผ่าน**, FAIL ข้อเดียว = รีวิวใหม่ทั้ง 5 ชั้น |
...
| | ใครทำ |
|---|---|
| ถ่ายภาพ + วงแดง + เขียนขั้นตอน | `manual-section-writer` **2–3 ตัว** คนละหัวข้อย่อย |
| รีวิวทันทีเมื่อหัวข้อย่อยเสร็จ | `manual-section-reviewer` **1 ตัว** (อ่านอย่างเดียว) |
| ล็อกอิน · อ่านแหล่งข้อมูล · ประกอบไฟล์ · รีวิว 5 ชั้น · เผยแพร่ | thread หลัก |
...
| `MANUAL_MAKER_PROGRESS_INTERVAL=<วินาที>` | เปลี่ยนช่วงเวลา (ค่าเริ่มต้น `600` = 10 นาที, ต่ำสุด 60) |
...
| งานไม่แตกเป็นหลายตัว ทำทีละหัวข้อเหมือนเดิม | ยังเป็น session เก่า (agent โหลดตอนเปิด session) **หรือ** รอบนั้นมีหัวข้อย่อยเดียว (ไม่แตกโดยตั้งใจ) | เปิด session ใหม่; เช็คว่า `manual-section-writer` / `manual-section-reviewer` อยู่ในลิสต์ agent |
| ไม่มีตารางความคืบหน้าโผล่เลย | ตัวเตือนเกาะจังหวะ tool call — ช่วงที่ไม่มี tool call เลยจะไม่ยิง **หรือ** ถูกปิดด้วย `MANUAL_MAKER_NO_PROGRESS=1` | ปกติ ไม่ใช่ error; สกิลยังรายงานตอนหัวข้อย่อยเปลี่ยนสถานะ. สั่ง "ขอสรุปความคืบหน้า" ได้ตลอด |
| ถูกถามบ่อยระหว่างทำ | ผู้รีวิวเจอเรื่องที่ตารางยืนยันไม่ได้ระบุ — **ตั้งใจให้ถาม ไม่ให้เดา** | ตอบให้ชัดครั้งเดียว; ลดคำถามรอบหน้าได้ด้วยการให้ **source + รายการคำล็อก** ครบตั้งแต่ intake |
